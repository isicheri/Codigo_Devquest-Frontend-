...
interface WithdrawFormProps {
  ...
  onWithdrawWithSlippage?: (amount: number, slippage: number) => Promise<void>
  isLoading: boolean
}

export default function WithdrawForm({
  onWithdraw,
  onWithdrawWithSlippage,
  isLoading,
}: WithdrawFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (useSlippage) {
      ...
      if (!onWithdrawWithSlippage) {
        setError("Slippage withdrawal is not supported")
        return
      }
      try {
        await onWithdrawWithSlippage(amt, slip)
        setAmount("")
        setSlippage("1")
      } catch (err: any) {
        setError(err.message || "Error withdrawing with slippage")
      }
    } else {
      try {
        ...
        setError(err.message || "Error withdrawing")
      }
    }
  }
  ...
  return (
    <form
      ...
      className="space-y-4 max-w-md p-4 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800"
    >
      <div>
